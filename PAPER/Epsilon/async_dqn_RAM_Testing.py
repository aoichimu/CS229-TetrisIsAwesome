...
flags.DEFINE_boolean('stats', True, 'If true, run tests for statistical purposes')
flags.DEFINE_string('stats_filename', 'Dummy_Name', 'Name for the file with the stats data')
FLAGS = flags.FLAGS
T = 0
TMAX = FLAGS.tmax
...
def statistics(session, graph_ops, num_actions, saver):
    stats_writer=open(FLAGS.stats_filename,'a+')
    times , paths =get_testing_data(FLAGS.checkpoint_dir,FLAGS.stats_filename)
    num_tests=len(times)
    if num_tests ==0:
        print "No new testing data to process, try again some other time"
    else:
        #stats_writer.write('T MIN MAX AVG MDN STD\n')
        for t in range(num_tests):
            ...
            for i_episode in xrange(FLAGS.num_eval_episodes):
                ...
                avg_reward+=ep_reward
            stats_writer.write('{0} {1} {2} {3} {4} {5}\n'.format(times[t],np.min(test_reward),np.max(test_reward),np.mean(test_reward),np.median(test_reward),np.std(test_reward)))
            avg_reward=avg_reward/FLAGS.num_eval_episodes
            print 'Average Reward RAM-Boltz: ' , avg_reward
    stats_writer.close()
...
def main(_):
  ...
  with g.as_default(), tf.Session() as session:
    ...
    if FLAGS.testing:
        ...
        statistics(session,graph_ops, num_actions, saver)
    else:
