...
def temperature(epsilon,f,num_actions):
    ...
    return np.log(num_actions*(1/epsilon-1)+1)/f
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions, summary_ops, saver):
    ...
    print "Starting thread ", thread_id, "with final epsilon ", epsilon
    t = 0
    while T < TMAX:
        ...
        while True:
            ...
            if max_q!=min_q:
                exp_values = 1.0*np.exp(np.clip(1.0*tau*readout_t[0]/(max_q-min_q), -500, 500))
                probs = 1.0*exp_values / np.sum(exp_values)
                probs[num_actions-1]=1.0-np.sum(probs)+probs[num_actions-1]
